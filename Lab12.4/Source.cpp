...
void EditTrain(const string& filename) {
    fstream file(filename, ios::binary | ios::in | ios::out);
    if (!file.is_open()) {
        cout << "Не вдалося відкрити файл для редагування!" << endl;
        return;
    }

    int index;
    cout << "Введіть індекс поїзда для редагування (від 0): ";
    cin >> index;

    file.seekg(index * sizeof(Train), ios::beg);
    Train train;
    ...
    cin.ignore();
    cout << "Редагування пункту призначення (старе: " << train.destination << "): ";
    cin.getline(train.destination, 50);

    int type;
    cout << "Редагування типу поїзда (старе: " << trainTypeStr[train.type] << "): ";
    cin >> type;
    while (type < 0 || type > 2) {
        cout << "Неправильний тип! Повторіть спробу: ";
        cin >> type;
    ...
    cout << "Редагування часу відправлення (старе: " << train.departureTime << "): ";
    cin >> train.departureTime;

    file.seekp(index * sizeof(Train), ios::beg);
    file.write((char*)&train, sizeof(Train));
    file.close();

    cout << "Поїзд успішно відредаговано." << endl;
}

void DeleteTrain(const string& filename) {
    ifstream file(filename, ios::binary);
    if (!file.is_open()) {
        cout << "Не вдалося відкрити файл для читання!" << endl;
        return;
    ...
    int index;
    cout << "Введіть індекс поїзда для видалення (від 0): ";
    cin >> index;
    ...
    while (file.read((char*)&train, sizeof(Train))) {
        if (currentIndex != index) {
            tempFile.write((char*)&train, sizeof(Train));
        }
        currentIndex++;
    }
    ...
    cout << "Поїзд успішно видалено." << endl;
}
